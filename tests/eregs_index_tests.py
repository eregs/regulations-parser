...
class VersionEntryTests(SetupMixin, TestCase):
...
class DependencyGraphTests(SetupMixin, TestCase):
    ...
    def test_nonexistent_files_are_stale(self):
        ...
        self.dgraph.add(self.depender, self.dependency)
        self.assertFalse(self.dgraph.is_stale(self.dependency))
        self.assertTrue(self.dgraph.is_stale(self.depender))
        # shouldn't raise an exception; all dependencies are up to date
        self.dgraph.validate_for(self.depender)

    def test_nonexistant_deps_are_stale(self):
        """If a dependency is not present, we're stale"""
        self.depender.write('value')
        self.dgraph.add(self.depender, self.dependency)
        self.assertTrue(self.dgraph.is_stale(self.dependency))
        self.assertTrue(self.dgraph.is_stale(self.depender))
    ...
    def test_updates_to_dependencies_flow(self):
        ...
        self.dgraph.add(self.depender, self.dependency)
        self.assertFalse(self.dgraph.is_stale(self.dependency))
        self.assertFalse(self.dgraph.is_stale(self.depender))
    ...
    def test_dependencies_serialized(self):
        ...
        dependencies"""
        self.dgraph.add(self.depender, self.dependency / '1')
        self.dgraph.add(self.depender, self.dependency / '2')
        self.assertEqual(
            self.dgraph.graph[str(self.depender)],
